...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CSTL);
    ...
    if (!settings.contains("nAnonymizeCastleAmount"))
        settings.setValue("nAnonymizeCastleAmount", 1000);

    nAnonymizeCastleAmount = settings.value("nAnonymizeCastleAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCastleAmount"))
        SoftSetArg("-anonymizecastleamount", settings.value("nAnonymizeCastleAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in castle.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCastleAmount:
            return QVariant(nAnonymizeCastleAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCastleAmount:
            nAnonymizeCastleAmount = value.toInt();
            settings.setValue("nAnonymizeCastleAmount", nAnonymizeCastleAmount);
            emit anonymizeCastleAmountChanged(nAnonymizeCastleAmount);
            break;
        case CoinControlFeatures:
