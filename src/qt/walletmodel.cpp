...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered castle address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CSTL.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal castle:URI (castle:XyZ...?message=example)
            {
...
static void NotifyzCSTLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCSTLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCSTLReset.connect(boost::bind(NotifyzCSTLReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCSTLReset.disconnect(boost::bind(NotifyzCSTLReset, this));
}
