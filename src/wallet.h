...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 40000 CSTL at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCSTL send
enum ZerocoinSpendStatus {
    ZCSTL_SPEND_OKAY = 0,                            // No error
    ZCSTL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCSTL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCSTL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCSTL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCSTL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCSTL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCSTL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCSTL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCSTL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCSTL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCSTL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
...
public:
    ...
    */
    ...
    /** zCSTL reset */
    boost::signals2::signal<void()> NotifyzCSTLReset;
};
